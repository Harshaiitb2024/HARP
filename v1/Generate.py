...
# --- Global Constants ---
LOOKUP_TABLE = [
    "hard_surface", "brickwork", "rough_concrete", "unpainted_concrete", "rough_lime_wash",
    "smooth_brickwork_flush_pointing", "smooth_brickwork_10mm_pointing", "brick_wall_rough",
    "ceramic_tiles", "limestone_wall", "reverb_chamber", "plasterboard", "wooden_lining",
    "glass_3mm", "glass_window", "double_glazing_30mm", "double_glazing_10mm", "wood_1.6cm",
    "curtains_cotton_0.5", "curtains_0.2", "curtains_velvet", "curtains_glass_mat", "carpet_cotton",
    "carpet_6mm_closed_cell_foam", "carpet_6mm_open_cell_foam", "carpet_tufted_9m", "felt_5mm",
    "carpet_hairy", "concrete_floor", "marble_floor", "orchestra_1.5_m2", "panel_fabric_covered_6pcf",
    "panel_fabric_covered_8pcf", "ceiling_fibre_abosrber"
]
...
def safe_measure_rt60(rir_list, fs):
    try:
        rt60 = pra.experimental.measure_rt60(rir_list, fs)
        return np.nanmax(rt60)
    except Exception:
        return 0.3  # fallback default (300 ms)
...
def simulate_room(room_idx: int, pos_idx: int, output_path: str, ambisonic_microphone, physical_params: list) -> Union[dict, None]:
    ...
    try:
        ...
        logging.error(f"Room {room_name} failed: {e}")
        return {
            "Name": npz_filename,
